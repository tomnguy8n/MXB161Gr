...
function nSurrounding = count_infected_neighbors(currentPos, agentPositions, agentStates, gridSize, infectionRadius, neighborhoodType)
    ...
    for i = 1:size(agentPositions, 1)
        if strcmp(neighborhoodType, 'Moore') || strcmp(neighborhoodType, 'Neumann')
            % Use the existing method for Moore and Neumann neighborhoods
        ...
        elseif strcmp(neighborhoodType, 'Euclidean')
            % Use Euclidean distance for the new neighborhood type
            distance = sqrt((currentPos(1) - agentPositions(i, 1))^2 + (currentPos(2) - agentPositions(i, 2))^2);
            if distance > 0 && distance <= infectionRadius && agentStates(i) == 1
                nSurrounding = nSurrounding + 1;
            end
        else
